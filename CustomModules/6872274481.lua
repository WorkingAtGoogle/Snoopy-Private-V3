...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/WorkingAtGoogle/Snoopy-Private-V3/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		assert(suc, res)
	...
	task.spawn(function()
		pcall(function()
			...
			if not isfile("vape/Profiles/bedwarsdata.txt") then 
				local commit = "main"
				for i,v in pairs(game:HttpGet("https://github.com/WorkingAtGoogle/Snoopy-Private-V3"):split("\n")) do 
				...
				writefile("vape/Profiles/bedwarsdata.txt", game:HttpGet("https://raw.githubusercontent.com/WorkingAtGoogle/Snoopy-Private-V3/"..commit.."/CustomModules/bedwarsdata", true))
			end
			...
			repeat
				...
				local newdata = game:HttpGet("https://raw.githubusercontent.com/WorkingAtGoogle/Snoopy-Private-V3/"..commit.."/CustomModules/bedwarsdata", true)
...
runFunction(function()
	local Old4bigguysAura = {["Enabled"] = false}
        Old4bigguysAura = GuiLibrary.ObjectsThatCanBeSaved.CombatWindow.Api.CreateOptionsButton({
        Name = "Old4bigguysExploit",
        Function = function(callback)
            if callback then
                task.spawn(function()
					repeat
						task.wait(0.03)
						if (GuiLibrary.ObjectsThatCanBeSaved["Lobby CheckToggle"]["Api"].Enabled == false or bedwarsStore.matchState ~= 0) and Old4bigguysAura["Enabled"] then
							local plrs = EntityNearPosition(18)
								if plrs then 
        ...
        ["HoverText"] = "Makes Other People Fly"
    })
end)
...
runFunction(function()
	...
	SwordHighlight = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		Name = "SwordHighlight",
		HoverText = "Don't have sword equipped when you turn it on.",
		Function = function(callback)
			if callback then
				RunLoops:BindToHeartbeat("DeathTp", function()
					sword_connection = gameCamera.Viewmodel.ChildAdded:Connect(function(obj)
						if obj:FindFirstChild("Handle") then
							...
							if obj_name:find("sword") or obj_name:find("blade") then
								...
								highlight_instance.OutlineColor =  Color3.fromRGB(105, 59, 255)
